...
To make it concrete: say the limit is 3 per second and requests came in at `t=0.5s`, `t=0.8s`, and `t=0.9s`. A new request arrives at `t=1.1s`. The timestamp at `t=0.5s` is now more than 1 second old, so it gets removed. The log has 2 entries. Request goes through.

<img
  src="/blogs/rate-limiting/wlog.png"
